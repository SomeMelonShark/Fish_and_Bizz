...
public class GoldfishEggEntity extends GoldfishFryEntity implements GeoEntity, EntitySize {
    private final AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
    public GoldfishEggEntity(EntityType<? extends GoldfishEntity> entityType, World world) {
    ...
    protected void growUp() {
        ...
        int i = random.nextBetweenExclusive(5, 8);
        for (int j = 1; j <= i; ++j)
            if (world instanceof ServerWorld) {
                ...
                if (m != 0) {
                    ...
                    pattern = random.nextBoolean() ? this.getPattern() : (ModUtil.getRandomTagValue(getWorld(), GoldfishPattern.Tag.PATTERNS, random));
                    detail = random.nextBoolean() ? this.getDetail() : (ModUtil.getRandomTagValue(getWorld(), GoldfishDetail.Tag.DETAILS, random));
                ...
                GoldfishFryEntity nextEntity = ModEntities.GOLDFISH_FRY.create(this.getWorld());
                if (nextEntity != null) {
    ...
    public ItemStack getBucketItem() {
        return new ItemStack(ModItems.GOLDFISH_EGG_BUCKET);
    }
