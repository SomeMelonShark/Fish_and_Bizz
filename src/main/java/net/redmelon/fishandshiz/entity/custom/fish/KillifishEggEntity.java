...
public class KillifishEggEntity extends KillifishEntity implements GeoEntity, EntitySize {
    private final AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
    public KillifishEggEntity(EntityType<? extends KillifishEntity> entityType, World world) {
        ...
        for (int j = 1; j <= i; ++j)
            if (world instanceof ServerWorld) {
                ...
                if (m != 0) {
                    ...
                    pattern = random.nextBoolean() ? this.getPattern() : (ModUtil.getRandomTagValue(getWorld(), KillifishPattern.Tag.PATTERNS, random));
                    detail = random.nextBoolean() ? this.getDetail() : (ModUtil.getRandomTagValue(getWorld(), KillifishDetail.Tag.DETAILS, random));
                ...
                KillifishFryEntity nextEntity = ModEntities.KILLIFISH_FRY.create(this.getWorld());
                if (nextEntity != null) {
    ...
    public ItemStack getBucketItem() {
        return new ItemStack(ModItems.KILLIFISH_EGG_BUCKET);
    }
