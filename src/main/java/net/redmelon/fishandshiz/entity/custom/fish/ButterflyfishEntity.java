...
public class ButterflyfishEntity extends SchoolingBreedEntity implements GeoEntity, VariableFishEntity<ButterflyfishPattern, ButterflyfishDetail>, EntitySize {
    private static final TrackedData<ButterflyfishPattern> PATTERN = DataTracker.registerData(ButterflyfishEntity.class, ButterflyfishPattern.TRACKED_DATA_HANDLER);
    private static final TrackedData<ButterflyfishDetail> DETAIL = DataTracker.registerData(ButterflyfishEntity.class, ButterflyfishDetail.TRACKED_DATA_HANDLER);
    private static final TrackedData<ModEntityColor> BASE_COLOR = DataTracker.registerData(ButterflyfishEntity.class, ModEntityColor.TRACKED_DATA_HANDLER);
    private static final TrackedData<ModEntityColor> PATTERN_COLOR = DataTracker.registerData(ButterflyfishEntity.class, ModEntityColor.TRACKED_DATA_HANDLER);
    private static final TrackedData<ModEntityColor> DETAIL_COLOR = DataTracker.registerData(ButterflyfishEntity.class, ModEntityColor.TRACKED_DATA_HANDLER);
    private static final TrackedData<NbtCompound> MATE_DATA = DataTracker.registerData(ButterflyfishEntity.class, TrackedDataHandlerRegistry.NBT_COMPOUND);
    ...
    public ItemStack getBucketItem() {
        return new ItemStack(ModItems.BUTTERFLYFISH_BUCKET);
    }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        setPattern(ButterflyfishPattern.fromId(nbt.getString("Pattern")));
        setDetail(ButterflyfishDetail.fromId(nbt.getString("Detail")));
    ...
    protected void initDataTracker() {
        ...
        dataTracker.startTracking(PATTERN, ButterflyfishPattern.BANDS);
        dataTracker.startTracking(DETAIL, ButterflyfishDetail.NONE);
        ...
        if (spawnReason == SpawnReason.NATURAL) {
            if (registryEntry.matchesKey(BiomeKeys.WARM_OCEAN) | registryEntry.matchesKey(BiomeKeys.LUKEWARM_OCEAN) | registryEntry.matchesKey(BiomeKeys.MANGROVE_SWAMP) | (biomeId != null && biomeId.getNamespace().equals("terrestria") && biomeId.getPath().equals("volcanic_island"))) {
                if (i == 0) {
                    ...
                    pattern = (ButterflyfishPattern.SLASHES);
                    detail = (ButterflyfishDetail.EYEGASH);
                    color = (ModEntityColor.SILVER);
                    color2 = (ModEntityColor.ORANGE);
                    color3 = (ModEntityColor.MATTE_BLACK);
                } else if (i == 2) {
                    pattern = (ButterflyfishPattern.ANNULAR);
                    detail = (ButterflyfishDetail.EYEGASH);
                    color = (ModEntityColor.SILVER);
                    color2 = (ModEntityColor.YELLOW);
                    color3 = (ModEntityColor.HADAL_BLUE);
                } else {
                    pattern = (ButterflyfishPattern.BRASS);
                    detail = (ButterflyfishDetail.EYESPOT);
                    color = (ModEntityColor.SILVER);
                    color2 = (ModEntityColor.ORANGE);
                    color3 = (ModEntityColor.MATTE_BLACK);
                }
            } else {
                pattern = (ModUtil.getRandomTagValue(getWorld(), ButterflyfishPattern.Tag.PATTERNS, random));
                detail = (ModUtil.getRandomTagValue(getWorld(), ButterflyfishDetail.Tag.DETAILS, random));
    ...
    public void setPattern(ButterflyfishPattern pattern) {
        dataTracker.set(PATTERN, pattern);
    }

    public ButterflyfishPattern getPattern() {
        return dataTracker.get(PATTERN);
    }

    public void setDetail(ButterflyfishDetail detail) {
        dataTracker.set(DETAIL, detail);
    }

    public ButterflyfishDetail getDetail() {
