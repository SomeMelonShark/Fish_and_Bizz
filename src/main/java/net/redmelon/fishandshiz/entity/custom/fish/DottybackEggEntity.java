...
public class DottybackEggEntity extends DottybackEntity implements GeoEntity, EntitySize {
    private final AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
    public DottybackEggEntity(EntityType<? extends DottybackEntity> entityType, World world) {
        ...
        for (int j = 1; j <= i; ++j)
            if (world instanceof ServerWorld) {
                ...
                if (m != 0) {
                    ...
                    pattern = random.nextBoolean() ? this.getPattern() : (ModUtil.getRandomTagValue(getWorld(), DottybackPattern.Tag.PATTERNS, random));
                    detail = random.nextBoolean() ? this.getDetail() : (ModUtil.getRandomTagValue(getWorld(), DottybackDetail.Tag.DETAILS, random));
                ...
                DottybackFryEntity nextEntity = ModEntities.DOTTYBACK_FRY.create(this.getWorld());
                if (nextEntity != null) {
    ...
    public ItemStack getBucketItem() {
        return new ItemStack(ModItems.DOTTYBACK_EGG_BUCKET);
    }
